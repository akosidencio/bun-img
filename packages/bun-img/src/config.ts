...
export interface ImageEngineConfig {
  ...
  /**
   * Snap widths and qualities onto the allowed lists. Default `true`.
   *
   * Turn it off when the caller controls the URL space and needs exact values.
   * Astro is the case that matters: an author writes `<Image width={300} />`,
   * Astro emits `width="300"` into the markup, and quantizing to 320 would make
   * the declared width disagree with the actual bytes.
   *
   * Leave it on for anything publicly addressable — it is the only real defence
   * against a cache-cardinality flood, and signed URLs are post-v1.
   */
  quantize?: boolean;
