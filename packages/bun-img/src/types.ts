...
export interface OptimizedImage {
  ...
  /** Where the bytes came from. `coalesced` means another request did the work. */
  readonly cache?: "hit" | "miss" | "coalesced";
  /** The cache key this result is stored under. */
  readonly key?: string;
  /**
   * Source version at the time of transform, when the source had one.
   *
   * Its absence is the signal that the engine cannot detect source changes, so
   * the HTTP layer must not claim `immutable` — that would pin a stale image in
   * shared caches for a year.
   */
